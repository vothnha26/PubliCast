    ...
    <div className="p-6 overflow-y-auto flex-1" style={{ fontFamily: "'DM Sans', sans-serif", background: "#F8F8F7" }}>
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button className="px-3 py-2 rounded-lg bg-white shadow-sm" style={{ fontSize: 12, border: "0.5px solid #E5E7EB", color: "#6B7280" }}>Last 12 months ▾</button>
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
         {/* MRR Breakdown */}
         <div className="bg-white rounded-[24px] p-6 col-span-2 shadow-sm" style={{ border: "0.5px solid #E5E7EB" }}>
         ...
         {/* Distribution */}
         <div className="bg-white rounded-[24px] p-6 shadow-sm flex flex-col items-center justify-center" style={{ border: "0.5px solid #E5E7EB" }}>
      ...
      {/* Recent Transactions */}
      <div className="bg-white rounded-[32px] overflow-hidden shadow-sm" style={{ border: "0.5px solid #E5E7EB" }}>
        <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
          <span style={{ fontSize: 15, fontWeight: 700, color: "#0A0A0A" }}>Recent Transactions</span>
          <button className="text-[11px] font-bold text-gray-400 hover:text-black uppercase tracking-widest transition-colors">View All</button>
        </div>
        <table className="w-full border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {data.transactions.length === 0 ? (
               <tr><td colSpan={7} className="px-8 py-10 text-center text-gray-400 text-xs font-bold uppercase">No transactions yet</td></tr>
            ) : (
              data.transactions.map((t, i) => (
                <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-8 py-5">
                    <button className="p-2 border border-gray-100 rounded-lg hover:bg-white hover:shadow-sm transition-all text-gray-400 hover:text-black">
