...
class InboxService {
  async getInbox(brandId, searchParamsString = '') {
    const data = await apiV2.get(`/inbox?brandId=${brandId}&${searchParamsString}`);
    return data;
  }

  async getThread(itemId) {
    const data = await apiV2.get(`/inbox/${itemId}`);
    return data;
  }

  async syncInbox(brandId, platform) {
    const data = await apiV2.post('/inbox/sync', { brandId, platform });
    return data;
  }

  async updateInboxStatus(itemId, status) {
    const data = await apiV2.patch(`/inbox/${itemId}/status`, { status });
    return data;
  }

  async replyInbox(payload) {
    const data = await apiV2.post('/inbox/reply', payload);
    return data;
  }

  async updateInboxReply(replyId, payload) {
    const data = await apiV2.patch(`/inbox/replies/${replyId}`, payload);
    return data;
  }

  async deleteInboxReply(replyId, brandId) {
    const data = await apiV2.delete(`/inbox/replies/${replyId}`, { data: { brandId } });
    return data;
  }

  async getAutoReplySettings(socialAccountId) {
    const data = await apiV2.get(`/inbox/auto-reply/settings/${socialAccountId}`);
    return data;
  }

  async saveAutoReplySettings(socialAccountId, payload) {
    const data = await apiV2.post(`/inbox/auto-reply/settings/${socialAccountId}`, payload);
    return data;
  }
