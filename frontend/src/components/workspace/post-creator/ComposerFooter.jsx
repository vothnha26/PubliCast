...
export function ComposerFooter({ isNetworkCustomize = false, onCloseNetworkCustomize }) {
  ...
  // scrolling horizontally instead of wrapping/shrinking keeps every
  // control (especially Publish) reachable rather than clipped off-screen.
  return (
    <div className="w-full shrink-0 px-3 md:px-8 py-3.5 border-t border-border/60 bg-card flex items-center justify-between gap-3 overflow-x-auto scrollbar-none z-30 shadow-[0_-4px_16px_-4px_rgba(0,0,0,0.04)]">
      ...
      <div className="flex items-center gap-3 shrink-0">
        {!isNetworkCustomize && !isLibrary && selectedAccountIds.length > 1 && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-composer-accent text-composer-accent-foreground text-xs font-bold rounded-xl transition-all cursor-pointer font-sans shadow-xs hover:opacity-90 shrink-0"
          >
            <span className="whitespace-nowrap">{t("planner:postCreator.footer.customizePerNetwork", "Customize for each network")}</span>
            <ArrowRight size={14} />
          </button>
        )}

        {!isLibrary && ['schedule', 'review'].includes(selectedPublishId) && (
          <label className="flex items-center gap-2 bg-card border border-border/60 rounded-xl px-3.5 py-2 hover:bg-muted transition-all relative cursor-pointer font-sans shadow-2xs shrink-0">
