...
export function ComposerBody() {
  ...
  // triggers for the same thing — both drive rightPanelTab, which
  // PostCreator.jsx uses to embed AICopilotPopover (variant="panel") in the
  // right column. No popover is rendered from this component anymore.
  const showAICopilot = rightPanelTab === 'ai';
  const setShowAICopilot = (open) => setRightPanelTab(open ? 'ai' : 'preview');
          ...
          {isLockedPlatformTab ? (
            ...
            <>
              <textarea
                ...
                className="w-full p-5 text-sm font-medium leading-relaxed outline-none min-h-[300px] resize-none font-sans bg-transparent placeholder-slate-400 text-slate-900"
                placeholder={
                  isThreadsTab
              ...
              {(isCustomTab || isThreadsCustom) && (
                <p className="px-5 pb-1 text-[10px] text-purple-600 font-bold font-sans">
          ...
          {activePlatform === 'facebook' && facebookType === 'album' ? (
            ...
            <>
              ...
              <MediaThumbnailGrid
                ...
                mediaPosterUrl={mediaThumbnailUrl}
                onEditImage={(index) => {
