...
class StockControllerV2 {
  /**
   * The 400 branch keeps v1's flat { message, items, total, page } shape
   * (not v2Error) — the frontend's empty-state UI reads items/total/page
   * directly even on the validation-failure path, same as v1.
   */
  ...
  importMedia = asyncHandler(async (req, res) => {
    ...
    v2Success(res, importedItem, 'Stock media imported to library successfully', 201);
  });
}

module.exports = new StockControllerV2();
