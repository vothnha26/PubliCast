...
class GeminiTranscriptionStrategy extends BaseTranscriptionStrategy {
  async transcribe(localFilePath, mimeType) {
    ...
    try {
      logger.debug(`[GeminiTranscriptionStrategy] Step 1: Initiating file upload for size=${fileSize}, mime=${mimeType}...`);
      ...
      logger.debug(`[GeminiTranscriptionStrategy] Step 2: Uploading file bytes...`);
      ...
      logger.debug(`[GeminiTranscriptionStrategy] File uploaded successfully: ${fileResourceName}, URI: ${fileData.uri}`);
      ...
      while (fileState === 'PROCESSING' && checkAttempts < 10) {
        logger.debug(`[GeminiTranscriptionStrategy] File is processing. Waiting...`);
      ...
      logger.debug(`[GeminiTranscriptionStrategy] Step 3: Prompting model to transcribe...`);
      ...
      logger.debug(`[GeminiTranscriptionStrategy] Transcription result received.`);
      try {
      ...
      if (fileResourceName) {
        logger.debug(`[GeminiTranscriptionStrategy] Step 4: Cleaning up remote file ${fileResourceName} on Gemini servers...`);
        try {
          await axios.delete(
            `https://generativelanguage.googleapis.com/v1beta/${fileResourceName}`,
            { headers: { 'x-goog-api-key': apiKey } }
          );
          logger.debug(`[GeminiTranscriptionStrategy] Remote file cleanup complete.`);
        } catch (delError) {
...
class MockTranscriptionStrategy extends BaseTranscriptionStrategy {
  async transcribe(localFilePath, mimeType) {
    logger.debug(`[MockTranscriptionStrategy] Transcribing mock file: ${localFilePath}`);
