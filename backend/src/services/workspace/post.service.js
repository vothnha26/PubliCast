...
class PostService {
  ...
  async getPostAnalytics(postId, brandId) {
    ...
    const prisma = require('../../config/prisma');
    const [history, shortLinks] = await Promise.all([
      prisma.postAnalyticsDailySnapshot.findMany({
        where: {
          postId: postId,
          brandId: brandId
        },
      ...
      // Tim cac link duoc rut gon trong caption hoac comment cua bai post nay
      prisma.linkItem.findMany({
        where: {
          smartLink: { brandId },
          isAutoGenerated: true,
          OR: post.caption ? [{ url: { in: post.caption.match(/https?:\/\/[^\s<]+/g) || [] } }] : []
        },
        select: {
          id: true,
          title: true,
