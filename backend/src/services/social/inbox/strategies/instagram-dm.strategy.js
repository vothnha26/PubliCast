...
class InstagramDMSyncStrategy extends BaseSyncStrategy {
  supports(platform) {
    return platform.toUpperCase() === PLATFORMS.INSTAGRAM;
  }
  ...
  async sync(brandId, inbox) {
    ...
    while (hasNextPage && limitPages > 0) {
      const response = await facebookGateway.getPageConversations(pageId, pageAccessToken, 'instagram', afterCursor);
...
module.exports = InstagramDMSyncStrategy;
